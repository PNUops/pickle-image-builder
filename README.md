...
템플릿을 플랫폼이 사용하려면 OS 카탈로그에 등록하는 별도 절차를 거칩니다.
...
- **기존 템플릿은 마지막에 지웁니다.** 내려받기와 검증, 이미지 커스터마이즈가 모두
  ...
  그대로 남깁니다.
- **VMID는 템플릿 대역만 받습니다.** 1000부터 9999까지가 그 대역이고, 벗어난 값은
  무엇이 실행되기도 전에 거부합니다.
...
- **한 번에 한 빌드만 실행됩니다.** 동시에 돌면 같은 내려받기 경로를 두고 겹칩니다.
- **빌드마다 매니페스트를 남깁니다.** 템플릿은 자기 재료를 기록하지 않아서, 번호와
  이름만으로는 어떤 업스트림 이미지가 들어갔는지 알 수 없습니다. `manifests/`에
  템플릿 번호마다 파일 하나를 두고, 같은 내용을 게스트 안에도 넣어 한참 뒤에 그 VM
  하나만 보고도 되짚을 수 있게 합니다.
- **접속 화면이 플랫폼의 말을 합니다.** 배포판 기본 화면은 벤더에 소식을 받아 오고
  구독을 광고하며 로그인마다 시스템 요약을 다시 계산합니다. 그 자리에 sudo 가 무엇을
  묻는지, 재부팅이 필요한지, 디스크가 찼는지를 놓습니다. 이미 만들어진 VM 의 접속
  화면은 바꿀 수 없으므로 낡지 않을 것만 적습니다.
- **게스트 계정은 클라우드 이미지의 기본 사용자를 그대로 사용합니다.** 플랫폼도 같은
...
- **sudo가 비밀번호를 묻는지는 빌드가 sudo에게 직접 물어 확인합니다.** `visudo`가 보는
  것은 문법뿐입니다. 규칙이 가리키는 그룹에 cloud-init이 기본 사용자를 넣지 않으면 그
  규칙은 아무에게도 적용되지 않고, include 지시자 아래에 규칙을 두는 배포판에서는
  드롭인이 그 줄을 이길 수 없습니다. 어느 쪽도 문법 검사에 걸리지 않으므로, 이미지를
  복제된 VM과 같은 상태로 만들어 두고 기본 사용자로 sudo를 실행해 봅니다. 그 전에 같은
  사용자에게 비밀번호 없는 규칙을 주고 sudo가 통과시키는 것을 먼저 확인합니다. 빌드
  환경 안에서 sudo가 다른 이유로 실패하는 것은 비밀번호를 물은 것과 구별되지 않기
  때문입니다.
...
- **작은 게스트가 감당할 수 없는 기본값을 조입니다.** 감시 파일 수 상한은 메모리에서
  ...
  있습니다. `/tmp`를 메모리에 올리는 배포판에서는 그 크기가 RAM에서 나오므로, 큰
  압축을 푸는 학생이 디스크가 빈 채로 메모리를 다 소진합니다.
...
빌드만으로는 콘솔에 나타나지 않습니다. 템플릿을 가리키는 OS 카탈로그 행을 심는 것은
운영자 등록 절차이고, 그 절차가 넣는 행은 비활성 상태로 들어옵니다. 관리자가 활성으로
바꾸기 전까지 이 템플릿은 신청 화면의 OS 목록에 없습니다.
...
| `SUDO_GROUP` | sudo 권한을 주는 그룹. 규칙이 계정이 아니라 그룹을 가리키므로 계정명이 달라져도 규칙이 살아 있습니다. 빌드가 그 그룹이 이미지에 있는지, cloud-init이 기본 사용자를 그 그룹에 넣는지 확인합니다 |
...
`guest/pickle-diag.sh`는 VM 안에서 실행하는 읽기 전용 진단 스크립트입니다. 부하,
메모리, 디스크, 실패한 서비스, 상위 프로세스, 네트워크 상태를 한 번에 출력해 지원
문의에 붙일 스냅샷을 만듭니다. 아무것도 바꾸지 않고 상주하지도 않으며, 기본
유틸리티만 사용하므로 어느 배포판 이미지에서든 동작합니다. 템플릿에 굽지 않는
이유도 같습니다 — 설치를 전제하지 않으므로 이미 만들어진 VM에서도 그대로 씁니다.

```sh
curl -fsSL https://raw.githubusercontent.com/PNUops/pickle-image-builder/main/guest/pickle-diag.sh | bash
```
