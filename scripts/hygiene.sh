...
# of cross-reference these rules forbid. Keep every copy identical.
#
...
hygiene_check() {
  ...
  if ! raw=$(hygiene_scan -HnI "$HYGIENE_TOKENS"); then rc=1; raw=""; fi
  hits=$(printf '%s' "$raw" | hygiene_match "$HYGIENE_TOKENS" || true)
  if [ -n "$hits" ]; then
